        ...
        {loading ? (
          ...
          items.map(item => (
            <div key={item.id} className="group relative overflow-hidden rounded-2xl border bg-white shadow-sm ring-1 ring-slate-100 transition-all hover:shadow-md">
              {/* Hero Badge */}
              {item.isHero && (
                <div className="absolute top-2 left-2 z-10 flex items-center gap-1 rounded-full bg-amber-400 px-2 py-0.5 text-[8px] sm:text-[9px] font-black uppercase tracking-wider text-white shadow">
                  <Star className="w-2 h-2 sm:w-2.5 sm:h-2.5" fill="white" /> Hero
                </div>
              )}
              ...
              <div className="p-2.5 sm:p-4">
                <div className="mb-2 sm:mb-3 text-[10px] sm:text-sm font-semibold text-slate-900 line-clamp-1">
                  {item.alt || "No description provided"}
                </div>


                <div className="flex items-center border-t pt-2.5 sm:pt-4 gap-1.5 sm:gap-2">
                  <button
                    onClick={() => setHero(item)}
                    title={item.isHero ? "Remove Hero" : "Set as Hero"}
                    className={`inline-flex items-center justify-center gap-1 rounded-lg px-2 py-1.5 sm:py-2 text-[9px] font-bold uppercase tracking-wider transition-all ${item.isHero
                        ? "bg-amber-100 text-amber-600"
                        : "bg-slate-50 text-slate-400 hover:bg-amber-50 hover:text-amber-500"
                      }`}
                  >
                    <Star className={`w-3 h-3 sm:w-3.5 sm:h-3.5 ${item.isHero ? "fill-amber-500" : ""}`} />
                    <span className="hidden sm:inline">{item.isHero ? "Hero" : "Set Hero"}</span>
                  </button>
                  <button
                    onClick={() => openEdit(item)}
                    className="flex-1 inline-flex items-center justify-center gap-1 sm:gap-2 rounded-lg bg-slate-50 py-1.5 sm:py-2 text-[10px] sm:text-xs font-bold text-slate-600 transition-colors hover:bg-blue-50 hover:text-blue-600"
                  >
                    <Edit2 className="w-3 h-3 sm:w-3.5 sm:h-3.5" /> <span className="hidden xs:inline">Edit</span>
                  </button>
                  <button
                    onClick={() => setDeleteModal({ isOpen: true, id: item.id })}
                    className="inline-flex h-7 w-7 sm:h-8 sm:w-8 items-center justify-center rounded-lg bg-slate-50 text-slate-400 transition-colors hover:bg-red-50 hover:text-red-600"
                  >
